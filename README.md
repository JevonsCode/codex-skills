...
| Luna Subagent Delegation | `skills/luna-subagent-delegation/SKILL.md` | Complete delegation runtime: authorization gate, Luna-first routing, concurrency safety, task/result contracts, recovery, and primary-agent verification. | **Use when:** you explicitly want a task delegated or worked on in parallel.<br>**Try:** `Use $luna-subagent-delegation to split [task] into bounded, independently verifiable subtasks. Keep decisions, integration, and final verification with the primary agent.`<br>**You get:** Luna-first delegation with isolated work and a primary-agent-verified result. |
